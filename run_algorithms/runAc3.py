...
class CspAc3Runner:
    ...
    def run_interactive_test(self, solve=True):
        """Runs a complete interactive test of the AC-3 algorithm if solve is True. If solve is False builds the CSP given by the user"""
    ...
    def _solve_and_display(self):
        ...
        try:
            solver = Csp(self.arcs, self.domains.copy(), self.constraints)
            result_domains, has_solution = solver.runAc3()
            ...
            self.domains = result_domains.copy()
        except Exception as e:
            print(f"\nError during solving: {e}")
    
    def return_csp(self):
        """
        This method returns an istance of Csp that has the same variables, domains and constraint of this one. It could be useful after running the AC-3 algorithm 
        """
        return Csp(self.arcs.copy(), self.domains.copy(), self.constraints.copy())
